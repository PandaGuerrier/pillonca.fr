...
export default function UsersTable({ users, roles, q, selectedRoles }: DataTableProps) {
  ...
  const [roleIds, setRoleIds] = React.useState<string[]>(selectedRoles ?? [])
  ...
  const columns: ColumnDef<Data.Users.User>[] = [
    ...
    },
    {
      ...
      accessorFn: (user) => user.roleUuid ?? '',
      header: t('users.index.table.columns.role'),
      cell: ({ row }) => {
        const { roleUuid } = row.original
        const role = roles.find((r) => r.value === roleUuid)
